...
from mock_api.prompts import get_ner_prompt, classification_prompt_template
from mock_api.kg_retrieval import kg_context_retrieval
...
class DomainRouter: 
    ...
    def _generate_entities(self, domain, input_question):
        ner_prompt = get_ner_prompt(domain, input_question)
        ...
        if self.is_server:
            ...
            entities = response.outputs[0].text
        
        return entities
    
    def _extract_entities(self, domain, entities):
        if domain == "music":
            entities = entities[entities.find("{"): entities.find("}") + 1]
            ...
            return {"entity_name" : [i for i in entities_dict.values()]}
        else: 
            raise ValueError("Invalid domain passed")
            
    def get_kg_info(self, input_question, domain):
        print("this is the input_question: ", input_question)
        entities = self._generate_entities(domain, input_question)
        print("this is the value of entities: " , entities)
        entities = self._extract_entities(domain, entities)
        print("this si the value of extracted entities: ", entities)
        print("this si the type of: ", type(entities["entity_name"]))
        kg_info = kg_context_retrieval(domain, entities)
        return kg_info        
...
class MyRAGModel:
    ...
    def initialize_models(self, llm_name, is_server, vllm_server):
        ...
        self.domain_router = DomainRouter(llm_name, is_server, vllm_server)
    ...
    def load_retriever(self):
        base_path = "/home/jupyter/CRAG/data/crag_task_3_dev_v4/"
        
        file_paths = [i for i in glob.glob(base_path + "*.jsonl")]
        ...
        for file_path in file_paths:
            ...
            batch_size = 5 ## TODO: add a custom batch_size
            ...
            for data in tqdm(batch_data):
                ...
                if self.vectorstore is None:
                    self.vectorstore = Chroma.from_documents(documents=docs, embedding=self.sentence_model)
                else:
    ...
    def batch_generate_answer(self, batch: Dict[str, Any]) -> List[str]:
        ...
        for _idx, interaction_id in enumerate(batch_interaction_ids):
            ...
            classification_prompt = classification_prompt_template(query)
            
            domain = self.classify_domain(classification_prompt)
            print("this si the domain of given input query: ", domain)
            
            kg_docs = self.domain_router.get_kg_info(query, domain)
            print("this is the value of kg_docs: ", kg_docs)
            
            kg_splits = self.text_splitter.split_documents(kg_docs)
...
#                           weights_for_different_modes=[0.6, 0.4,0])
    ...
    def classify_domain(self, prompt):
        ...
        if self.is_server:
            ...
            domain = response.outputs[0].text
